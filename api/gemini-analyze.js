...
출력 언어 규칙(반드시 지킬 것): keywords의 word와 summary는 **예외 없이 한국어**로 작성하세요.
- 리뷰 원문이 영어 등 외국어여도 키워드는 한국어로 번역해서 넣으세요. 예) "delicious" → "맛있음", "friendly staff" → "친절한 직원", "long wait" → "긴 대기".
- 한국어로 굳어진 외래어(파스타, 라멘, 티라미수 등)는 그대로 한글 표기로 쓰고, 로마자 알파벳은 쓰지 마세요.
- 고유명사(가게 이름, 메뉴명)도 한글 표기로 옮겨 적으세요.
